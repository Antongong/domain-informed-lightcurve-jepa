...
# reads straight from the bootstrap pipeline's output, so this always reflects
# whatever get_predictions.py / bootstrap_metrics.py last produced
...
for ax, clf_name in zip(axes, CLASSIFIER_ORDER):
    ...
    for train_pct in train_levels:
        sub = clf_df[clf_df["train_pct"] == train_pct].set_index("test_pct")
...
fig.savefig("f1_vs_test_truncation_with_errorbars.png", dpi=300, bbox_inches="tight")
plt.show()
